...
impl Material {
    ...
    fn scatter_dielectric(ray: &Ray, hit: &Hit, refractive_index: f64) -> Option<(Ray, ColorRgb)> {
        ...
        let direction = if sin * directed_ri <= 1. && Self::reflectance(cos, directed_ri) <= rand::rng().random() {
    ...
    pub fn scatter(&self, ray: &Ray, hit: &Hit) -> Option<(Ray, ColorRgb)> {
        match self {
            Material::Lambertian { albedo } => {
                Self::scatter_lambertian(hit, albedo)
            }
            Material::Metal { albedo, fuzz } => {
                Self::scatter_metal(ray, hit, albedo, *fuzz)
            }
